...
def get_type_scores(kb_entities, kb_relations, neg_samples_kb, positive_pairs, negative_pairs, opts):
    ...
    entity_type = Embedding(output_dim=type_dim, input_dim=num_entities, init='normal', name='entity_type_embeddings')
    relation_head_type = Embedding(output_dim=type_dim, input_dim=num_relations, init='normal', name='relation_head_type_embeddings')
    relation_tail_type = Embedding(output_dim=type_dim, input_dim=num_relations, init='normal', name='relation_tail_type_embeddings')
    ...
    positive_pair_dot = Lambda(lambda x:T.batched_dot(x[:, 0], x[:, 1]), output_shape=(type_pair_count,))(positive_type_vectors)
    negative_pair_dot = Lambda(lambda x:T.batched_dot(x[:, 0], x[:, 1]), output_shape=(type_pair_count,))(negative_type_vectors)
...
def build_typed_model(opts, model_func):
    ...
    score_model, score_model_e1_corrupted, score_model_e2_corrupted = model_func(kb_entities, kb_relations, neg_samples_kb, opts)
    ...
    score = merge([score_model, score_type], lambda x:x[0]*x[1], output_shape=(1,))
    score_e1_corrupted = merge([score_model_e1_corrupted, score_type_e1_corrupted], lambda x:x[0]*x[1], output_shape=(neg_samples,))
    score_e2_corrupted = merge([score_model_e2_corrupted, score_type_e2_corrupted], lambda x:x[0]*x[1], output_shape=(neg_samples,))
    ...
    type_scores = merge([positive_pair_dot, negative_pair_dot], mode=lambda x:T.mean(negative_pair_dot)-T.mean(positive_pair_dot), output_shape=(1,))
    alg = get_optimizer(opts)
    model = Model([kb_entities, kb_relations, neg_samples_kb, positive_pairs, negative_pairs], output=[score_kb, type_scores])
    def lossf(y_true, y_pred):
        return T.mean(y_pred[0])+T.mean(y_pred[1])
    model.compile(loss=lossFn, optimizer=alg)
    return model
...
emp.neg_samples=200
emp.vect_dim=50
emp.num_entities=150
emp.num_relations=50
emp.optimizer='Adam'
emp.l2_entity=0
emp.l2_relation=0
emp.type_dim=20
